<template>
  <div v-if="!phoneIsVerified">
    <h4 class="">Phone Verification
      <span class="fa fa-times">
    </span></h4>
    <div>
      <p class="text-dark">A token will be sent to your phone number.</p>
      ...
      <small v-if="reqMessage && !reqError" class="px-2 text-success">
        Check your phone for the token and enter it below
      </small>
  ...
  <div v-else>
    <h4 class="mt-5">Your Phone number has been verified</h4>
  </div>
</template>
...
export default {
  ...
  methods: {
    handleRequest() {
      if (this.requestCount){
        ...
        UserService.requestPhoneVer().then(
          response => {
    ...
    handleSubmit() {
      ...
      this.$store.dispatch('profile/verifyPhone', this.token).then(
        response => {
          this.loading = false;
  ...
  computed: {
    phoneIsVerified: function (){
      return this.$store.getters["profile/phoneVerified"]
    }
  }
}
</script>
